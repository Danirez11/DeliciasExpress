...
async function cargarProductosAdmin() {
  ...
  try {
    const res = await fetch('https://deliciasexpress-1.onrender.com/productos');
...
async function eliminarProducto(id) {
  ...
  try {
    const res = await fetch(`https://deliciasexpress-1.onrender.com/productos/${id}`, { method: 'DELETE' });
    const data = await res.json();
